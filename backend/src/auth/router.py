...
from auth.service import create_user, get_current_user, authenticate_user, create_access_token, hash_password
...
async def register(req: RegisterRequest, db: AsyncSession = Depends(get_db)):
    try:
        ...
        # ✅ Отправляем email только если SKIP_EMAIL_VERIFICATION=false
...
async def forgot_password(req: ResetRequest, db: AsyncSession = Depends(get_db)):
    ...
    if user:
        ...
        await db.commit()
        log.info(f"Reset token for {req.email}: {user.reset_token}")
