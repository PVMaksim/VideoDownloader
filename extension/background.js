// VideoGrab — Background Service Worker v0.9
...
// ── webRequest для GetCourse / Kinescope ─────────────────────────
...
// ── При смене вкладки — проверяем YouTube/VK/Instagram ───────────
function checkPageVideo(tabId, tabUrl, tabTitle) {
  const pageVideo = getPageVideo(tabUrl, tabTitle);
  if (!pageVideo) return;

  const tab = getTab(tabId);
  // Добавляем только если ещё нет видео с этим URL страницы
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status === "complete" && tab.url && tab.url.startsWith("http")) {
    // Чистим старые видео при новой странице
    state[tabId] = { videos: [], seenUrls: new Set() };
    updateBadge(tabId, 0);

    // Проверяем YouTube/VK/Instagram
    checkPageVideo(tabId, tab.url, tab.title);

    // Performance scan для GetCourse
...
// При обновлении заголовка (YouTube меняет title после загрузки)
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
...
function performanceScanner() {
  ...
  function scan() {
    performance.getEntriesByType("resource").forEach(function(e) {
      if (isVideo(e.name)) window.dispatchEvent(new CustomEvent("__vg_found__", { detail: e.name }));
    });
  }
  try {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "GET_VIDEOS") {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      // Проверяем ещё раз — вдруг не поймали при загрузке
